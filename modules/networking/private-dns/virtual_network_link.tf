...
resource "azurerm_private_dns_zone_virtual_network_link" "vnet_links" {
  ...
  #virtual_network_id    = try(var.vnets[each.value.lz_key][each.value.vnet_key].id, var.vnets[var.client_config.landingzone_key][each.value.vnet_key].id, each.value.virtual_network_id)
